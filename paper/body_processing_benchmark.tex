...
For each task-dataset record, five trajectories are selected from the corresponding external evaluation dataset. The model predicts actions from the recorded observations of each trajectory, and the prediction error is measured against the associated human-teleoperation actions. The task-level result is obtained by averaging the trajectory-level MSE values over the five trajectories. Both checkpoints are evaluated on the same trajectories, providing a paired comparison between the no-training baseline and the Baihu-trained model.
...
The benchmark reports two mean squared error metrics. For each trajectory, the policy predicts an action sequence from the recorded observations. The predicted action values are compared with the corresponding demonstration actions from the evaluation dataset. For a selected set of action dimensions and evaluated timesteps, MSE is the average squared difference between the predicted action and the dataset action.
...
\begin{table}[t]
\centering
\caption{Per-platform offline zero-shot evaluation results on datasets external to Baihu. The table reports the unweighted mean over paired task-dataset records for each evaluated platform. Lower MSE is better.}
...
Task-level results further show that Baihu training improves offline prediction across the 42 external paired task-dataset records. Task-level reporting is especially useful because different platforms may have different action spaces, control frequencies, and gripper or hand dimensions.
